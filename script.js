const APIKEY = "fdbfcf88cb113edecaab164c46d51766" //"d562feeaf7ad62452461ee883ea94f51"
var restDetails;
var details;
// * API 
...
document.getElementById("searchByLocation").addEventListener("click", () => {
    ...
    if (id !== null) {
        ...
        try {
            ...
            popularLocations = Object.keys(details).slice(0, 7) //
        } catch {
...
let showRestaurants = () => {
    for (i = 0; i < 5; i++) {
        ...
        div1 = createElmAndSetAttr({
            ...
            'style': 'color:grey',
            'id': data.id,
            'onClick': 'getRestDetails(this.id)',
            'data-toggle': 'modal',
...
let createZomatoFooter = () => {
    ...
    div.append(rowdiv)
    let Details = {
        'COMPANY': ['Who we are', 'Blog', 'Careers', 'Report Fraud', 'Contact'],
    ...
    hr = createElmAndSetAttr({}, "hr")
    div.append(hr)

    p = createElmAndSetAttr({}, "p")
...
let getRestDetails = (id) => {
    // console.log(id)
    getRestaurantDetails(id).then((data) => {
        restDetails = data
        // console.log(restDetails)
        openRestaurant(restDetails)
    }).catch((err) => {
        // console.log(err)
    })
}
...
let openRestaurant = (restDetails) => {
    // console.log(restDetails)
    document.getElementById("modalRestName").innerText = restDetails.name
    document.getElementById("modalRestImage").href = restDetails.photos_url
    document.getElementById("modalImg2").src = restDetails.featured_image
    document.getElementById("modalRestMenu").href = restDetails.menu_url
    document.getElementById("modalSpan1").innerText = restDetails.location.address
    document.getElementById("modalP1").innerText = "  " + restDetails.phone_numbers
    document.getElementById("modalCost").innerText = "  " + restDetails.average_cost_for_two + " Avg cost for two"

    modal = document.getElementById("modalhighlights")
    for (i = 0; i < restDetails.highlights.length; i++) {
        div = createElmAndSetAttr({
            "class": "btn btn-outline-info p-1 m-2"
        }, "span")
        div.innerText = restDetails.highlights[i]
        modal.append(div)
    }

    modal = document.getElementById("modalCuisines")
    let cuisines = restDetails.cuisines.split(",")
    for (i = 0; i < cuisines.length; i++) {
        div = createElmAndSetAttr({
            "class": "btn btn-outline-success p-1 m-2"
        }, "span")
        div.innerText = cuisines[i]
        modal.append(div)
    }
    return restDetails
}
